/*
 * mylogin_after_5.c — Preparatory Assignment 5
 *
 * Builds on mylogin_after_4.c (failed/age tracking) by adding:
 *   - Signal handling: Ctrl-C (SIGINT) is ignored so the login program
 *     cannot be interrupted by the user. This is how real login programs
 *     behave — you can't Ctrl-C out of /bin/login.
 *
 * Change from after_4: Added signal.h include and signal(SIGINT, SIG_IGN) in main.
 *                      Also improved read_username to handle the NULL that fgets
 *                      returns when a signal interrupts input (defensive coding
 *                      now that signals are being handled).
 *
 * To kill this program when testing: press Ctrl-Z to suspend it, then run:
 *   sudo killall -9 mylogin
 *
 * Compile: gcc -o mylogin mylogin_after_5.c pwdblib.c -lcrypt
 */
...
#include <signal.h>  /* signal()  — for registering signal handlers (NEW in prep 5) */
 ...
 *
 * NEW in prep 5: handles NULL return from fgets. When SIGINT is ignored,
 *   fgets can still return NULL if stdin gets into an error state (e.g. Ctrl-C
 *   on some systems sets an error flag on stdin). We call clearerr() to reset
 *   the stream and set username to empty string so the main loop just re-prompts.
 */
...
{
    /*
     * === NEW IN PREP 5: Ignore SIGINT (Ctrl-C) ===
     *
     * signal() registers a handler for a given signal.
     *   - SIGINT is the signal sent when the user presses Ctrl-C.
     *   - SIG_IGN is a special handler that means "ignore this signal".
     *
     * Real login programs (like /bin/login) do this because it would be a
     * security hole if users could Ctrl-C out of the login prompt to get
     * a shell without authenticating.
     *
     * To kill this program during testing, use Ctrl-Z (suspend) then:
     *   sudo killall -9 mylogin
     */
