...
struct pwdb_passwd *pwdb_getpwnam(const char *name)
{
  ...
  { /* hmmm...try to create it instead */
    ...
    { /* if we just created the file, there aren't many users in it..*/
      fclose(fd);
...
int pwdb_update_user(struct pwdb_passwd *p)
{
  ...
  { /* hmmm...try to create it instead */
    fd = fopen(PWFILENAME, "w+");
    if (!fd)
    { /* what is going on here?..Bail out  */
      return (PWDB_FILEERR);
    }
  }
...
const char *pwdb_err2str(int errno)
{
  ...
  {
  case PWDB_NOUSER:
    return (strPWDB_NOUSER);
  case PWDB_FILEERR:
    return (strPWDB_FILEERR);
  case PWDB_MEMERR:
    return (strPWDB_MEMERR);
  case PWDB_ENTRERR:
    return (strPWDB_ENTRERR);
  case PWDB_NULL:
    return (strPWDB_NULL);
  default:
    return (strPWDB_UNKNOWNERR);
  }
}
